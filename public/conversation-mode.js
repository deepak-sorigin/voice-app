...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const speaker1Transcription = document.getElementById(
    "speaker1-transcription"
  );
  const speaker2Transcription = document.getElementById(
  ...
  function populateLanguageDropdowns() {
    ...
    const sourceLanguages = Array.from(sourceLanguageElem.options).map(
      (option) => ({ code: option.value, name: option.textContent })
    );

    console.log(
      "Found",
      sourceLanguages.length,
      "languages in source dropdown"
    );
    ...
    if (
      previousSpeaker1Lang &&
      sourceLanguages.some((lang) => lang.code === previousSpeaker1Lang)
    ) {
    ...
    if (
      previousSpeaker2Lang &&
      sourceLanguages.some((lang) => lang.code === previousSpeaker2Lang)
    ) {
  ...
  function handleWebSocketMessage(event) {
    try {
      ...
      if (message.type === "transcription") {
        ...
        const transcriptionElem =
          activeSpeaker === 1 ? speaker1Transcription : speaker2Transcription;
        ...
        if (message.data && message.data.isFinal) {
          const statusElem =
            activeSpeaker === 1 ? speaker1Status : speaker2Status;
      ...
      if (message.type === "translation") {
        ...
        const translationElem =
          activeSpeaker === 1 ? speaker2Translation : speaker1Translation;
      ...
      if (message.type === "error") {
        ...
        if (message.data && message.data.code === "AUDIO_TIMEOUT") {
          console.log(
            "Audio timeout occurred, stopping recording automatically"
          );
          ...
          notification.textContent =
            "Recording automatically stopped due to silence";
  ...
  async function startRecording(speakerId) {
    try {
      ...
      const sourceLanguage =
        speakerId === 1 ? speaker1Language.value : speaker2Language.value;
      ...
      console.log(
        "Starting recording with source language:",
        sourceLanguage,
        "and target language:",
        targetLanguage
      );
  ...
  async function toggleSpeaking(speakerId) {
    ...
    if (isSpeaking) {
      ...
      if (activeSpeaker) {
        const currentActiveToggle =
          activeSpeaker === 1 ? speaker1Toggle : speaker2Toggle;
        ...
        if (currentActiveToggle.classList.contains("stop")) {
          ...
          const currentActivePanel =
            activeSpeaker === 1 ? speaker1Panel : speaker2Panel;
      ...
      const transcriptionElem = isFirstSpeaker
        ? speaker1Transcription
        : speaker2Transcription;
